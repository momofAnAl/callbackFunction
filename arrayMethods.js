...
let taxRate = 0.07;
const postTaxPrices = prices.map((price) => price * (1 + taxRate));
...
const totalPopulation = populations.reduce((runningTotal, curValue) => runningTotal + curValue, initialValue);
...
const totalPostTaxPrice = orders.map((totalPrice) => totalPrice['price'] * (1 + totalPrice['tax']));
console.log(totalPostTaxPrice);
...
const priceBobs = purchases.filter((Bobs) => Bobs['owner'] === 'Bob');
// console.log(priceBobs);
const bobsTotal = priceBobs.reduce((runningTotal, curValue) => runningTotal + curValue['price'], 0);
